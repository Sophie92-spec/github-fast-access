...
async function rwp(d,k,t=3000){const p=P[k],c=new AbortController();const id=setTimeout(()=>c.abort(),t);try{const r=await fetch(p.url(d),{headers:p.headers,signal:c.signal});clearTimeout(id);const d2=await r.json();return d2.Answer?d2.Answer.filter(a=>a.type===1).map(a=>a.data):[]}catch(e){clearTimeout(id);throw e}}
async function rd(d,pref){if(pref!=='auto'){for(const p of A.filter(x=>x!==pref).concat(pref)){try{const ips=await rwp(d,p);if(ips.length>0)return{ips}}catch(e){}}return{ips:[]}}
// auto mode: race all providers, use fastest
return Promise.race(A.map(p=>rwp(d,p).then(ips=>ips.length>0?{ips}:null).catch(()=>null))).then(r=>r||{ips:[]})}
