...
页面顶部就有一张「DoH 延迟对比」卡片，把 DoH 源选择和「获取 IP + 测 IP 延迟」的所有按钮收在一起了。不同网络环境下各家 DoH 的速度差很多，所以给了一个对比工具，帮你挑当前网络里最快的那个：

1. 点卡片里的「测 DoH 延迟」，同时测 DNSPod、阿里、Cloudflare、Google、Quad9、AdGuard、360 这 7 个源解析 `github.com` 的耗时
2. 测完顶部下拉**自动切到最快的源**，不用再手动点
3. 表格里**绿框 = 本次最快**（推荐用），**红框 = 本次最慢**（避开），淡红底 = 测了但超时，前缀蓝点 ✓ = 当前下拉选中的源
4. 表格下方有一行小汇总：可用源数 / 超时数 / 慢源数 / 平均延迟 / 推荐源
5. 紧接着在同一张卡里点「获取 IP」，就用刚才选的最快源去查 GitHub 一堆域名的 IP
6. 想看每条 IP 的延迟，再点「测 IP 延迟」；测完域名表也会给**最快那条加绿框、最慢那条加红框**，跟 DoH 表视觉一致

两个状态位互不干扰：DoH 状态（左下小字）和 IP 状态（获取 IP 那行的右侧）分开显示，不会再混在一起。

**视觉同款**：DoH 表和域名表共用一套强烈态样式——绿框最快 / 红框最慢 / 淡红超时 / 蓝点当前。两表一眼看上去是一致的延迟语言，不会让人误会是两种数据。
