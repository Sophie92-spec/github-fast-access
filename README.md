...
2. 点击「生成 PowerShell 脚本」→「复制脚本」，把内容复制到剪贴板；
3. 打开记事本，粘贴内容，另存为 `github-hosts-apply.ps1`（保存类型选“所有文件”，编码 UTF-8）；
4. **右键**该 `.ps1` 文件 →「使用 PowerShell 运行」（需管理员权限）；
5. 脚本自动完成备份、写入、刷新 DNS；
6. 重新打开浏览器访问 GitHub 即可。

> 为什么用复制而非下载：Windows / Edge 对 `.ps1`、`.txt`、`.zip` 等任何形式的脚本/压缩下载都可能弹“可能会损害你的设备”安全警告。复制脚本内容到剪贴板不涉及文件下载，可彻底规避该提示。
