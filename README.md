...
页面顶部就有一张「DoH 延迟对比」卡片，把 DoH 源选择和「获取 IP + 测延迟」的所有按钮都收在一起了。不同网络环境下各家 DoH 的速度差很多，所以给了一个对比工具，帮你挑当前网络里最快的那个：

1. 点卡片里的「测 DoH 延迟」，同时测 DNSPod、阿里、Cloudflare、Google、Quad9、AdGuard、360 这 7 个源解析 `github.com` 的耗时
2. 表格里绿框标的是本次最快的源，标「当前」的是顶部下拉正在用的那个
3. 点某一行的「选用」，或直接点「选用最快」，顶部下拉立即切到那个最快的源
4. 紧接着在同一张卡里点「获取 IP」，就全用这个最快的源去查 GitHub 一堆域名的 IP
5. 想看每条 IP 的延迟，再点「测延迟」
